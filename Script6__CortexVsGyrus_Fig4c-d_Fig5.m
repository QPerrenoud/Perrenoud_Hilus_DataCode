...
for iPGp = 1:inNPGp
    ...
    for iIGp = 1:inNIGp
        ...
        [db1DotP(iIGp), db1PVal_DotP(iIGp), db2CI_DotP(:, iIGp), db2Score_H0, db2Score_Tst] = ...
            PCAAngleTest(db2DataH0_Tst, db2Data_Tst);
...
%         continue
        ...
        for iFig = 1:length(hFIG)
            exportfig(hFIG(iFig) ,fullfile(chGroupFolder, cFIG_LABEL{iFig}), 'Color', 'rgb', 'Renderer', 'painters');
            ...
            print(hFIG(iFig), fullfile(chGroupFolder, cFIG_LABEL{iFig}), '-dpdf')
            pause(.05)
        end
        close all
